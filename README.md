...
## 📌 現在の実装ステータス（今日時点の状態）
- [x] **主要機能①：画像のドラッグ＆ドロップ・プレビュー表示機能【本日完了】**
- [x] ユーザー入力のバリデーション（PNG, JPG, JPEG限定）およびエラーハンドリング
- [ ] 主要機能②：K-means法による5色の代表色抽出機能（未実装）
    - ※開発中仕様変更：背景の白・黒が占有率1位にならないよう、「背景色を無視するチェックボックス」をUIに追加予定（案Bを採用）
- [ ] 主要機能③：各色の面積比率の算出とPlotlyによるグラフ表示機能（未実装）
- [ ] 主要機能④：カラーコードのクリップボードへの一括コピー機能（未実装）
